...
def __pub_twist_cov0(v, w):
    ...
    twist_cov.child_frame_id = 'tb2_0_tf/odom'
...
def callback_twist1(twist_data):
    global v, w, T
    v = twist_data.linear.x
    w = twist_data.angular.z
    T = twist_data
    __pub_twist_cov1(twist_data.linear.x, twist_data.angular.z)

# Publish TwistWithCovarianceStamped, basically it just add the stamp, seq, covariance, and frame_id

def __pub_twist_cov1(v, w):

    global seq
    seq = seq + 1
    __pubTwist = rospy.Publisher('/twist1', TwistWithCovarianceStamped, queue_size=1)
    twist_cov = TwistWithCovarianceStamped()
    twist_cov.header.stamp = rospy.get_rostime()
    # Not sure if the header frame_id map is correct
    twist_cov.header.frame_id = 'map'
    twist_cov.child_frame_id = 'tb2_1_tf/odom'
    twist_cov.header.seq = seq
    twist_cov.twist.twist.linear.x = v
    # twist_cov.twist.twist.linear.y = float_list[3] * knots * math.sin(latest_yaw)
    twist_cov.twist.twist.angular.z = w
...
def callback_twist2(twist_data):
    global v, w, T
    v = twist_data.linear.x
    w = twist_data.angular.z
    T = twist_data
    __pub_twist_cov2(twist_data.linear.x, twist_data.angular.z)

# Publish TwistWithCovarianceStamped, basically it just add the stamp, seq, covariance, and frame_id

def __pub_twist_cov2(v, w):

    global seq
    seq = seq + 1
    __pubTwist = rospy.Publisher('/twist2', TwistWithCovarianceStamped, queue_size=1)
    twist_cov = TwistWithCovarianceStamped()
    twist_cov.header.stamp = rospy.get_rostime()
    # Not sure if the header frame_id map is correct
    twist_cov.header.frame_id = 'map'
    twist_cov.child_frame_id = 'tb2_2_tf/odom'
    twist_cov.header.seq = seq
    twist_cov.twist.twist.linear.x = v
    # twist_cov.twist.twist.linear.y = float_list[3] * knots * math.sin(latest_yaw)
    twist_cov.twist.twist.angular.z = w
...
if __name__ == "__main__":
    ...
    try:
        ...
        #sub_odom_filter = rospy.Subscriber('/odometry/filtered_map0', Odometry, callback_odom_filter)

        #sub_odom1 = rospy.Subscriber('/tb2_0/odom', Odometry, callback_odom0)

        sub_twist0 = rospy.Subscriber('/tb2_0/cmd_vel_mux/input/teleop', Twist, callback_twist0)
        sub_twist1 = rospy.Subscriber('/tb2_1/cmd_vel_mux/input/teleop', Twist, callback_twist1)

        sub_twist2 = rospy.Subscriber('/tb2_2/cmd_vel_mux/input/teleop', Twist, callback_twist2)
        rospy.spin()
